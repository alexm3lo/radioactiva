 ...
 * ----------------------------------------------------------------------
 * 				Upgrading functions for Themify themes and framework
 *  			http://themify.me
	...
	if( is_object($newF) || is_object($newT) ){
		...
		if( is_object($newT) && ($theme_version < $newT->version) ){
			...
			if($_GET['page'] == 'themify')
				$notifications .= sprintf( __('<p class="update %s">%s version %s is now available. <a href="%s" title="" class="%s" target="%s">Update now</a> or view the <a href="http://themify.me/logs/%s-changelogs" title="" class="" target="_blank">change log</a> for details.</p>', 'themify'), $newT->login, $theme_name, $newT->version, $newT->url, $newT->class, $newT->target, strtolower($theme_name));
			else
				$notifications .= '<p class="update">' . sprintf(__('%s version %s is now available. Visit the <a href="admin.php?page=themify">%s page</a> to update.', 'themify'), $theme_name, $newT->version, $theme_name) . '</p>';
		}
		...
		if( is_object($newF) && ($version < $newF->version) ){
			...
			if($_GET['page'] == 'themify')
				...
				$notifications .= '<p class="update">' . sprintf( __('Framework version %s is now available. Visit the <a href="admin.php?page=themify">%s page</a> to update.', 'themify'), $newF->version, $theme_name) . '</p>';
		}
			...
			if($update['_a']['name'] == 'themify' && !is_object($newF)){
				...
				if(str_replace(".","",$theme_version) < $latest){
					...
					if( function_exists('unzip_file') ){
						...
						$class = "";
						$target = "blank";
						$url = "http://themify.me/files/".strtolower($theme_name)."/".strtolower($theme_name).".zip";
					}
					$notifications .= sprintf( __("<p class='update %s'>%s version %s is now available. <a href='%s' title='' class='%s' target='%s'>Update now</a> or view the <a href='http://themify.me/logs/%s-changelogs' title='' class='' target='_blank'>change log</a> for details.</p>", 'themify'), $login, $theme_name, $update['_v'], $url, $class, $target, strtolower($theme_name));
...
function themify_updater(){
	global $theme;
	$themeName = strtolower($theme->display('Name'));
	$type = $_GET['type'];
