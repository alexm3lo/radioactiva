...
/***************************************************************************/	
	...
	function themify_header_slider($data=array()){
		...
		/** Slider status */
		if('' == $data['setting-header_slider_enabled'] || 'on' == $data['setting-header_slider_enabled']){
		...
		foreach ($slider_ops as $key => $val) {
			$output .= '<option value="'.$val.'" ' . selected($data['setting-header_slider_enabled'], $val, false) . '>' . $key . '</option>';
		}
		$output .= '</select>' . '</p>
					...
					<p class="pushlabel feature_box_posts" '.$display_posts_display.'>';
							$output .= wp_dropdown_categories(array("show_option_all"=>__('All Categories', 'themify'),"hide_empty"=>0,"echo"=>0,"name"=>"setting-header_slider_posts_category","selected"=>$data['setting-header_slider_posts_category']));
		...
		$output .= '<div class="feature_box_images" '.$display_images_display.'>';
				...
				foreach($image_options as $option){
					...
					<div class="uploader-fields">
						...
						<input type="text" class="width10 feature_box_img" id="setting-header_slider_images_'.$option.'_image" name="setting-header_slider_images_'.$option.'_image" value="'.$data['setting-header_slider_images_'.$option.'_image'].'" /> 
